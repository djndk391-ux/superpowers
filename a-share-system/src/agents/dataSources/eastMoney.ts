...
export class EastMoneyAdapter extends BaseDataSourceAdapter {
    ...
    try {
      ...
      try {
        ...
        if (response && response.data && response.data.klines && response.data.klines.length > 0) {
          ...
          northFlow = parseFloat(latestData.split(',')[1]) * 100000000;
        }
  ...
  private getMockSectors(): any[] {
    return [
      { id: 'ai', name: '人工智能', change: 5.8 + (Math.random() - 0.5) * 2, vol: 158000000000 + (Math.random() - 0.5) * 20000000000, leaders: ['300223', '300418', '002230'], source: this.name, timestamp: new Date().toISOString() },
      { id: 'newenergy', name: '新能源', change: 3.2 + (Math.random() - 0.5) * 1.5, vol: 98000000000 + (Math.random() - 0.5) * 15000000000, leaders: ['300750', '002594', '000333'], source: this.name, timestamp: new Date().toISOString() },
      { id: 'consumerelec', name: '消费电子', change: 2.5 + (Math.random() - 0.5) * 1, vol: 76000000000 + (Math.random() - 0.5) * 10000000000, leaders: ['002475', '300136', '002241'], source: this.name, timestamp: new Date().toISOString() },
      { id: 'medicine', name: '生物医药', change: 1.8 + (Math.random() - 0.5) * 1, vol: 65000000000 + (Math.random() - 0.5) * 8000000000, leaders: ['300142', '600276', '000661'], source: this.name, timestamp: new Date().toISOString() },
      { id: 'finance', name: '金融', change: -0.5 + (Math.random() - 0.5) * 0.5, vol: 52000000000 + (Math.random() - 0.5) * 6000000000, leaders: ['601318', '600036', '601166'], source: this.name, timestamp: new Date().toISOString() },
      { id: 'realestate', name: '房地产', change: -1.2 + (Math.random() - 0.5) * 0.8, vol: 38000000000 + (Math.random() - 0.5) * 5000000000, leaders: ['000002', '600048', '000656'], source: this.name, timestamp: new Date().toISOString() }
    ];
  }
  ...
  private getMockNews(): any[] {
    ...
    return [
      { id: 'news_1', title: 'AI大模型应用落地加速，产业链持续受益', content: '多家上市公司宣布AI大模型应用落地，行业景气度持续提升，相关龙头公司订单饱满，产能利用率高', source: '证券时报', time: today.toISOString(), tags: ['AI', '大模型', '利好'], dataSource: this.name },
      { id: 'news_2', title: '新能源汽车销量再创新高', content: '5月份新能源汽车销量同比增长超60%，产业链需求旺盛，上游原材料价格企稳回升', source: '第一财经', time: yesterday.toISOString(), tags: ['新能源', '销量', '数据'], dataSource: this.name },
      { id: 'news_3', title: '消费电子迎来传统旺季', content: '下半年消费电子传统旺季临近，产业链备货积极性提升，相关公司订单改善', source: '上海证券报', time: yesterday.toISOString(), tags: ['消费电子', '旺季', '需求'], dataSource: this.name }
    ];
  }
}
